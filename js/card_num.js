...
$(function () {
  ...
  let $tip = $(".alert");
  let cn = null;
  function initCmbRegions () {
    const select = document.getElementById('cmbRegionSelect');
    Object.keys(regionCodes).forEach(region => {
  ...
  async function calc () {
    ...
    const selectedCmbRegion = $("#cmbRegionSelect").val();
    ...
    await cn.forEachOnlyLuhnValid(c => {
      ...
      // 招行地区筛选逻辑
      if (cmbCodeCheckbox || selectedCmbRegion) {
        const middleThreeDigits = c.replace(/\s/g, "").slice(6, 9);
        let valid = false;
        let regionName = "";

        // 优先使用下拉框选择
        if (selectedCmbRegion) {
          if (regionCodes[selectedCmbRegion]?.includes(middleThreeDigits)) {
            valid = true;
            regionName = selectedCmbRegion;
          }
        }
        // 其次使用复选框筛选
        else if (cmbCodeCheckbox) {
  ...
  // 防止重复绑定事件
  $(document).off('change', '#cmbCodeCheckbox, #cmbRegionSelect')
    .on('change', '#cmbCodeCheckbox, #cmbRegionSelect', calc);
  function calCardLen () {
    var val = $("#card").val();
    val = val.replace(/[ \t]+/g, "");
